...
export default class ForgotPassword extends React.Component{
    ...
    validateEmail(email) {
      ...
      click(){
        ...
        Axios.post("http://localhost:8009/forgotpassword/uic",data).then(res => this.setState({status:res.data.status,msg:'please enter registered email'}));
        ...
        return(
            <div>
                <h2>Please enter your email to recover the password</h2>
                    ...
                    {this.state.status==='true'?<Methods email={document.getElementById('email').value}/>:''}
            </div>
        );
    }
}